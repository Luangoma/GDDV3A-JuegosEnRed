...
function addMainCamera(scene)
{
	scene.cameras.main.setBounds(0, 0, 2048, 2048);
	scene.physics.world.setBounds(0, 0, 2048, 2048);
	scene.cameras.main.startFollow(map_test_1_variables.player1.sprite, true);
...
function addSplitScreenCamera(scene, split_type = 0)
{
	if(split_type === 0)
	{
		scene.cameras.add(0,0,config.width,config.height/2).startFollow(map_test_1_variables.player1.sprite,true).setBounds(0,0,world_width, world_height);
		scene.cameras.add(0,config.height/2,config.width,config.height/2).startFollow(map_test_1_variables.player2.sprite,true).setBounds(0,0,world_width, world_height);
	}
	else
	{
		scene.cameras.add(0,0,config.width,config.height/2).startFollow(map_test_1_variables.player1.sprite,true).setBounds(0,0,world_width, world_height);
