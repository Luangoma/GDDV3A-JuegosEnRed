...
function generateTilesList()
{
	let ans = [world_tiles_width * world_tiles_height];
	return ans;
}

function fillTilesList(list)
{
	for(let i = 0; i < list.length; ++i)
	{
		list[i] = {has_house: true, tile_skins: getRandomHouseTiles()};
	}
}

function generateHouseData(spawn_list, num_houses)//warning: this function shuffles the list given to it, so this is one of the first steps that should be done
{
	//generar una lista aleatoria donde se determina las posiciones aleatorias de las casas que tienen que spawnear en el mundo.
	//la lista también necesita aleatorizar el tipo de casas que se van a generar
	//NOTA: no se hace uso de un rand en el spawneo de casas directamente para garantizar que el numero de casas spawneadas sean exactamente "num_houses". Rand solo se utiliza para determinar de forma aleatoria las skins de las casas y sus posiciones (internamente dentro de shuffleList), pero NO se utiliza para determinar el número de casas ni la probabilidad de spawneo de casas.
	
	let num_tiles = world_tiles_width * world_tiles_height;
	let obj_to_push = {has_house: false, tile_skins: []};
	for(let i = 0; i < num_houses; ++i){
		obj_to_push = {
			has_house: true,
			tile_skins: getRandomHouseTiles()
		};
		spawn_list.push(obj_to_push);
	}
	for(let i = 0; i < num_tiles - num_houses; ++i){
		obj_to_push = {
			has_house: false,
	...
	shuffleList(spawn_list); //se utiliza shuffle para garantizar que las casas estén en posiciones aleatorias
	
	return spawn_list;
}

function generateShortTilesList(spawn_list)
{
	let ans = [];
	
	for(let i = 0; i < spawn_list.length; ++i){
		if(spawn_list[i].has_house){
			let obj = {
				index: i,
				tile_skins: spawn_list[i].tile_skins
			};
			ans.push(obj);
		}
	}
...
//creates the houses by copying data from a list of predetermined spawn points
function createHousesFromList(scene, background_tiles, tiles, flamesgroup, spawn_list)
{
	let global_index = 0;
	for(let i = 0; i < world_height ; i+=128)
	{
		for(let j = 0; j < world_width; j+=128)
		{
			//utilizar el índice global de la casilla en la lista donde está en memoria para determinar si debe de spawnear una casa o no, además de las skins que tendrá la casa.
			let current_obj = spawn_list[global_index];
			let should_spawn = current_obj.has_house;
			let current_skins = current_obj.tile_skins;
			++global_index;
			if(!should_spawn)
			{
				continue;
			...
			let current_tile = new Tile(scene, 'house_tile_1_d0', j, i, flamesgroup ? true : false, 300 /*TODO: add hp config later*/, flamesgroup);
			current_tile.tiles = current_skins;
			current_tile.tile = current_tile.tiles[0];
			background_tiles[global_index - 1].sprite.setTexture('ground_tile_2');
...
//creates the houses by copying data from a list of predetermined spawn points, but this time using a lighter weight list that only contains the non empty tiles.
function createHousesFromShortList(scene, background_tiles, tiles, flamesgroup, spawn_list)
{
	let global_index = 0;
	let list_index = 0;
	for(let i = 0; i < world_height ; i+=128)
	{
		...
		{
			//utilizar el índice global de la casilla en la lista donde está en memoria para determinar si debe de spawnear una casa o no, además de las skins que tendrá la casa.
			let current_obj = spawn_list[list_index];
			let should_spawn = list_index < spawn_list.length && global_index === current_obj.index;
			++global_index;
			if(!should_spawn)
			{
				continue;
...
//Modifies the data in a data list generated by the function create houses from list and other such functions that generate the full WORLD_WIDTH * WORLD_HEIGHT array instead of a simplified one
//adds a rectangular fill with the type of tiles that it is told to generate.
function rectangleFillData(tiles, is_house, tiletypes, xmin, ymin, xmax, ymax)
{
	for(let i = ymin; i <= ymax; ++i)
	{
		for(let j = xmin; j <= xmax; ++j)
		{
			let pos = j + i * (world_tiles_width);
			let current_htile = {
				has_house: is_house,
				tile_skins: tiletypes
			};
			tiles[pos] = current_htile;
			console.log("filled idx: " + pos);
		}
	}
}
...
function createCastleData(tiles, x, y)
{
	let castle_front = ['c_01_f_d0','c_01_f_d1','c_01_f_d2','c_01_f_d3'];
	let castle_side = ['c_01_s_d0','c_01_s_d1','c_01_s_d2','c_01_s_d3'];
	...
	//corner top left
	rectangleFillData(tiles, true, castle_corner_tl, x, y, x, y);
	//corner top right
	rectangleFillData(tiles, true, castle_corner_tr, x+3, y, x+3, y);
	//corner bottom left
	rectangleFillData(tiles, true, castle_corner_bl, x, y+3, x, y+3);
	//corner bottom right
	rectangleFillData(tiles, true, castle_corner_br, x+3, y+3, x+3, y+3);
	
	//side top
	rectangleFillData(tiles, true, castle_front, x+1, y, x+2, y);
	//side right
	rectangleFillData(tiles, true, castle_side, x+3, y+1, x+3, y+2);
	//side bottom
	rectangleFillData(tiles, true, castle_front, x+1, y+3, x+2, y+3);
	//side left
	rectangleFillData(tiles, true, castle_side, x, y+1, x, y+2);
	
	
	//inner parts (placeholder, will be its own castle components in the future) TODO: Replace with 1 single rectangle fill
	//corner top left
	rectangleFillData(tiles, true, getRandomHouseTiles(), x+1, y+1, x+1, y+1);
	//corner top right
	rectangleFillData(tiles, true, getRandomHouseTiles(), x+2, y+1, x+2, y+1);
	//corner bottom left
	rectangleFillData(tiles, true, getRandomHouseTiles(), x+1, y+2, x+1, y+2);
	//corner bottom right
	rectangleFillData(tiles, true, getRandomHouseTiles(), x+2, y+2, x+2, y+2);
	
}


//the objects in the tile_list list are of type ---> obj: {is_destructible: true/false, tiles: [x,y,z,w]}
//the purpose of this function is to facilitate world creation from network data, which is why it does not contain a full tile object and instead sends a more light-weight object with just the right amount of required data.
function createTilesFromList(scene, tiles, tile_list = [])
{
	for(let i = 0; i < world_tiles_height ; ++i)
	{
		for(let j = 0; j < world_tiles_width; ++j)
		{
			let current_tile_from_list = tile_list[j * world_tiles_width + i];
			let current_tile = new Tile(scene, current_tile_from_list.tiles[0], current_tile_from_list.x, current_tile_from_list.y, current_tile_from_list.is_destructible, 300);
			current_tile.tiles = tile_list.tiles;
			tiles.push(current_tile);
			current_tile.create();
		}
	}
